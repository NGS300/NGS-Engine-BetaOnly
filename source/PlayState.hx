...
class PlayState extends MusicBeatState{
	...
	private function popUpRating(note:Note):Void{
		...
		if (ratingShit != 'shit' || ratingShit != 'bad'){
			...
			ratingSprite.velocity.y -= FlxG.random.int(160, 200); // old 140, 175
			ratingSprite.velocity.x -= FlxG.random.int(0, 15); // old 0, 10
			if (!TrackMap.curMap.startsWith('pix')){
		...
		if (grabbediconP2){
			...
			switch (curTrack){
				case 'Roses':
					if (mainDifficulty == 3)
						if (curStep >= 64)	
							HealthAction('HealthDrain', 0.015);
			}
		}
		if (!playerTurn){
			if (opponentHitedCounter % 100 == 0){
	...
	override function stepHit(){
		...
		switch (curTrack){
			case 'Cocoa':
				if (dad.curCharacter == 'parents-christmas')
					switch (curStep){
						case 130 | 385 | 576:
